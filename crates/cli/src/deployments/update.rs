...
use slot_graphql::deployments::update_deployment::{self, UpdateServiceInput};
use slot_graphql::deployments::{update_deployment::*, UpdateDeployment};
        ...
        let service = match &self.update_commands {
            UpdateServiceCommands::Katana(args) => {
                let config = if let Some(config) = args.config.clone() {
                    ...
                    Some(slot_utils::base64_encode_string(&service_config))
                } else {
                    None
                };
