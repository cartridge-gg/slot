...
use slot_utils::get_selector_from_name;
use slot_utils::NonAsciiNameError;
...
pub enum SessionError {
    ...
    #[error(transparent)]
    Credentials(#[from] slot_core::credentials::CredentialsError),

    #[error(transparent)]
    SignError(#[from] account_sdk::signers::SignError),
...
///
pub fn get(chain: Felt) -> Result<Option<FullSessionInfo>, SessionError> {
...
/// This function will return an error if there is no authenticated user.
///
pub fn store(chain: Felt, session: &FullSessionInfo) -> Result<PathBuf, SessionError> {
...
pub async fn create(
...
) -> Result<FullSessionInfo, SessionError> {
    ...
    let methods = policies
        ...
        .map_err(SessionError::InvalidMethodName)?;
...
fn get_at(
    ...
    chain: Felt,
) -> Result<Option<FullSessionInfo>, SessionError> {
...
mod tests {
    use super::*;
    use super::{get_at, get_user_relative_file_path, store_at};
