...
use slot_core::credentials::AccessToken;
use slot_utils::vars;
use url::Url;
...
impl Client {
    ...
    pub async fn query<R, T>(&self, body: &T) -> Result<R, ClientError>
    where
        R: DeserializeOwned,
        T: Serialize + ?Sized,
    {
        ...
        if response.is_err() {
            return Err(ClientError::ReqwestError(response.err().unwrap()));
        }

        let res = response?;

        if res.status() == 403 {
            return Err(ClientError::InvalidOAuth);
        }

        if !res.status().is_success() {
            return Err(ClientError::Anyhow(anyhow::anyhow!(
                "API error: {}",
                res.status()
            )));
        }
        ...
        if let Some(errors) = res.errors {
            Err(ClientError::GraphQL(GraphQLErrors(errors)))
        } else {
            Ok(res.data.unwrap())
        }
    }

    pub async fn oauth2(&self, code: &str) -> Result<AccessToken, ClientError> {
        #[derive(Deserialize)]
        struct OauthToken {
