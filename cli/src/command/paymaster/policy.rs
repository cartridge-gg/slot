...
struct RemovePolicyArgs {
    ...
    paymaster_id: String,
    #[arg(long, required = true, value_delimiter = ',', help = "Comma-separated list of policy IDs to remove.")]
...
impl PolicyCmd {
    ...
    async fn run_add(args: &AddPolicyArgs) -> Result<()> {
        ...
        let file_content = fs::read_to_string(&args.policies_file)
            .context(format!("Failed to read policies file: {:?}", args.policies_file))?;
        let policies_json: Vec<PolicyInputJson> = serde_json::from_str(&file_content)
            .context(format!("Failed to parse policies JSON from file: {:?}", args.policies_file))?;
