        ...
        if let Some(me) = data.me {
            if let Some(teams_edges) = me.teams.edges {
                ...
                let paymasters_data = teams_edges
                    .iter()
                    .filter_map(|team_edge_opt| team_edge_opt.as_ref()) // Get &TeamEdge from Option<TeamEdge>
                    .filter_map(|team_edge| team_edge.node.as_ref()) // Get &TeamNode from Option<TeamNode>
                    ...
                    .flat_map(|(team_node, pm_edges)| {
                        ...
                        pm_edges
                            .iter()
                            .filter_map(|pm_edge_opt| pm_edge_opt.as_ref()) // Get &PaymasterEdge from Option<PaymasterEdge>
